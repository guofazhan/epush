...
import static com.epush.apns.ApnsConfigure.APNS_TOPIC_HEADER;
...
public class ApnsPushNotification {
	 ...
	 */
	public Http2Request toHttp2Request(Host host, final P8 p8)
			throws SignatureException {
		...
		if (p8 != null) {
			httpHeaders.add(ApnsConfigure.APNS_AUTHORIZATION_HEADER,
					"bearer " + p8
							.getAuthenticationTokenSupplierForTopic(getTopic())
							.getToken());
		} else {
		}
        ...
        if (getTopic() != null) {
            httpHeaders.add(APNS_TOPIC_HEADER, getTopic());
        }

		// 构建一个HTTP2请求
		return new Http2Request(getPayload(), httpHeaders, host,
				ApnsConfigure.APNS_PATH_PREFIX + getToken());
	}
