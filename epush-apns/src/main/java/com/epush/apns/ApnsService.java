...
public final class ApnsService
	...
	public PushNotificationResponse push(ApnsPushNotification pushNotification)
		...
		try {
			...
			Logger.APNS.error("Push Error : ", e);
	...
	protected Http2Request buildHttp2Request(
			...
			throws SignatureException {
		if (Logger.APNS.isDebugEnabled()) {
			Logger.APNS.debug(
					"Build Http2 Request from ApnsPushNotification {}",
	...
	protected PushNotificationResponse parseHttp2Response(
		...
		if (Logger.APNS.isDebugEnabled()) {
			Logger.APNS.debug(
					"Parser Http2 Response {} To PushNotificationResponse",
					http2Response);
		}
