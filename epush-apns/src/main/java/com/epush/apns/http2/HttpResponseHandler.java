...
public class HttpResponseHandler extends SimpleChannelInboundHandler<FullHttpResponse> {
     ...
     * @return
     */
    public FullHttpResponse getResponse(int streamId, long timeout, TimeUnit unit) {
        ...
        if (null != entry) {
            ...
            if (!writeFuture.awaitUninterruptibly(timeout, unit)) {
                throw new Http2Exception(new IllegalStateException("Timed out waiting to write for stream id " + streamId));
                ...
                if (!responsePromise.awaitUninterruptibly(timeout, unit)) {
                    throw new Http2Exception(new IllegalStateException("Timed out waiting for response on stream id " + streamId));
...
//                    if (null == response) {
//                        response = new Http2Response<String>("Not find response on stream id " + streamId, null);
//                        response.setException(new Http2Exception("Not find response on stream id " + streamId));
//                    }
                }
            }
