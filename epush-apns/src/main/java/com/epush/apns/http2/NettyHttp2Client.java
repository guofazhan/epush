...
public abstract class NettyHttp2Client implements Client {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(NettyHttp2Client.class);

	final AttributeKey<RequestContext> requestKey = AttributeKey
			.newInstance("request");
	...
	public void request(final RequestContext context) throws Http2Exception {
		String host = context.host.getHost();
		int port = context.host.getPort();
		// 2.添加请求超时检测队列
		...
		if (channel == null) {
			...
			f.addListener(new ChannelFutureListener() {
				public void operationComplete(ChannelFuture channelFuture)
						throws Exception {
					LOGGER.debug("create new channel cost={}",
					...
					if (channelFuture.isSuccess()) {
						...
						LOGGER.warn("create new channel failure, request={}",
								context);
					}
				}
			});
	...
	private void writeRequest(Channel channel, RequestContext context) {
		...
		pool.attachHost(context.host.getHost(), channel);
		channel.writeAndFlush(context.getRequest())
				.addListener(new ChannelFutureListener() {
					public void operationComplete(ChannelFuture channelFuture)
							throws Exception {
						if (!channelFuture.isSuccess()) {
							RequestContext info = channelFuture.channel()
									.attr(requestKey).getAndSet(null);
							info.tryDone();
	...
	public void init() throws Http2Exception {
		...
		// this.bootstrap.channelFactory(SocketChannelClassUtil
		// .getServerSocketChannelClass(workerGroup));
		bootstrap.handler(getHttp2ClientInitializer());
	 ...
	 * @return
	 */
	protected abstract Http2ClientInitializer getHttp2ClientInitializer();
