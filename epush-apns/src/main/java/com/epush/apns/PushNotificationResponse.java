...
public class PushNotificationResponse {
	...
	private static final Gson gson = new GsonBuilder()
			.registerTypeAdapter(Date.class,
					new DateAsTimeSerializerAdapter(TimeUnit.MILLISECONDS))
			.create();
	 ...
	 * @return
	 */
	public static PushNotificationResponse build(Http2Response http2Response) {
		...
		if (http2Response.getException() != null) {
			pushNotificationResponse.setRejectionReason(
					http2Response.getException().getMessage());
			pushNotificationResponse.setSuccess(false);
		} else if (http2Response.getHttpResponse() != null) {
			final HttpResponseStatus status = http2Response.getHttpResponse()
					.status();
			if (status.equals(HttpResponseStatus.OK)) {
				...
				// 请求失败
				pushNotificationResponse.setSuccess(false);
				final String content = http2Response.getResponse();
				if (content != null) {
					final ErrorResponse errorResponse = gson.fromJson(content,
							ErrorResponse.class);
					if (errorResponse != null) {
                        pushNotificationResponse
                                .setRejectionReason(errorResponse.getReason());
                        pushNotificationResponse.setTokenExpirationTimestamp(
                                errorResponse.getTimestamp());
					}
                    else{
                        pushNotificationResponse
                                .setRejectionReason(content);
                    }
				} else {
					pushNotificationResponse
							.setRejectionReason(status.toString());
				}
